---
layout: profiles
permalink: /people/
title: people
description: Meet the members and collaborators of the CIDER Lab!
nav: true
nav_order: 3

profile_sections:
  - title: PI
    profiles:
      - align: left
        image: people/jasonwu.png
        image_circular: true
        name: Jason Wu
        link: https://jasonwu.cc
        role: Assistant Professor
        affiliation: Purdue University
  - title: Purdue Undergraduates
    profiles:
      - align: left
        image: people/james.png
        image_circular: true
        name: James Qiao Sun
        link: https://www.linkedin.com/in/jamesqsun/
        role: Undergraduate Researcher
        affiliation: Purdue University
      - align: left
        image: people/rishab.png
        image_circular: true
        name: Rishab Vempati
        link: https://rishabvempati.com/
        role: Undergraduate Researcher
        affiliation: Purdue University
      - align: left
        image: people/saran.png
        image_circular: true
        name: Saran Nagubandi
        link: https://www.linkedin.com/in/saran-nagubandi/
        role: Undergraduate Researcher
        affiliation: Purdue University
      - align: left
        image: people/samarth.png
        image_circular: true
        name: Samarth Das
        link: https://www.linkedin.com/in/sdas44/
        role: Undergraduate Researcher
        affiliation: Purdue University
      - align: left
        image: people/aiden.jpg
        image_circular: true
        name: Aiden Moy
        role: Undergraduate Researcher
        affiliation: Purdue University
  - title: Collaborators
    profiles:
      - align: left
        image: people/yihao.png
        image_circular: true
        name: Yi-Hao Peng
        link: https://www.yihaopeng.tw/
        role: PhD Collaborator
        affiliation: Carnegie Mellon University
      - align: left
        image: people/alice.png
        image_circular: true
        name: Alice Gao
        link: https://atgao.github.io/
        role: PhD Collaborator
        affiliation: University of Washington
      - align: left
        image: people/longjie.png
        image_circular: true
        name: Longjie Guo
        link: https://longjieguo.github.io/
        role: PhD Collaborator
        affiliation: University of Washington
      - align: left
        image: people/donghoon.png
        image_circular: true
        name: Donghoon Shin
        link: https://donghoon.io/
        role: PhD Collaborator
        affiliation: University of Washington
      - align: left
        image: people/ziying.jpg
        image_circular: true
        name: Ziying Wang
        role: Undergraduate Collaborator
        affiliation: Duke Kunshan University
---
